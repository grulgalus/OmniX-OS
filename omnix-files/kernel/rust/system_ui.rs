...
static mut BG_COLOR: u8 = 3; // Cyan pozadí (Windows 95 styl)
...
static mut WINDOWS: [Window; 4] = [
    Window { id: 1, x: 20, y: 15, w: 250, h: 150, title: b"TERMINAL", visible: false },
...
unsafe fn handle_desktop_clicks(mx: usize, my: usize, clicked: bool) {
    ...
    // Kliknutí na Start tlačítko
    if mx >= 2 && mx <= 47 && my >= 185 && my <= 198 { 
        START_MENU_OPEN = !START_MENU_OPEN; 
        return; 
    }

    // Pokud je Start menu otevřené
    if START_MENU_OPEN {
        if mx >= 2 && mx <= 112 && my >= 70 && my <= 185 {
            ...
            else if my > 160 && my < 175 { /* Tady by byl Shutdown */ } 
        }
        START_MENU_OPEN = false; // Zavřít po kliknutí kamkoliv
        return;
    }
    ...
    for i in (0..4).rev() { 
        ...
        if w.visible && mx >= w.x && mx <= w.x + w.w && my >= w.y && my <= w.y + w.h {
            ACTIVE_WIN = i; 
            if mx >= w.x + w.w - 14 && my <= w.y + 14 { w.visible = false; } // Zavírací X
            
            // Měnění barvy pozadí v Settings
            if w.id == 4 {
...
unsafe fn handle_keyboard(key: u8, pressed: bool) {
    ...
    // Terminál
    if active_id == 1 { 
        if key == 8 { if TERM_LEN > 0 { TERM_LEN -= 1; } } // Backspace
        else if key == b'\n' { process_command(); } // Enter
...
fn draw_desktop() {
    ...
    // Tlačítko Start (zmáčknuté, když je menu otevřené)
    ...
    vga::draw_str(b"12:00", 282, 189, 0);
}

fn draw_start_menu() {
    // Win95 Hlavní šedý box
    draw_raised_rect(2, 70, 110, 115, 7);
    
    // Levý tmavě modrý pruh (klasický Win95 prvek)
    vga::draw_rect(4, 72, 20, 111, 1);
    
    // Svislý text OMNIX OS
    vga::draw_str(b"O", 10, 130, 15);
    vga::draw_str(b"M", 10, 140, 15);
    vga::draw_str(b"N", 10, 150, 15);
    vga::draw_str(b"I", 10, 160, 15);
    vga::draw_str(b"X", 10, 170, 15);
    
    // Položky menu
    vga::draw_str(b"TERMINAL", 30, 78, 0);
    vga::draw_str(b"FILES", 30, 98, 0);
    vga::draw_str(b"NOTES", 30, 118, 0);
    vga::draw_str(b"SETTINGS", 30, 138, 0);
...
unsafe fn draw_app_window(w: &Window, is_active: bool) {
    vga::draw_rect(w.x + 2, w.y + 2, w.w, w.h, 0); // Stín okna
    draw_raised_rect(w.x, w.y, w.w, w.h, 7); // Rám okna
